export class InMemoryFetcher {
    constructor() {
        this.data = moviesData.map((movie) => ({
            ...movie,
    ...
    async get(url, params) {
        ...
        if (params && Object.keys(params).length > 0) {
            const filtered = this.data.filter((movie) => Object.entries(params).every(([key, value]) => movie[key]?.toString().toLowerCase().includes(value.toString().toLowerCase())));
            return filtered;
        }
        return this.data;
    }
...
//# sourceMappingURL=InMemoryFetcher.js.map